...
def download_omni_data(year):
    ...
    with open(f'omni_5min_{year}.lst.', 'wb') as f:
...
def download_omni_low_res_data(year):
    ...
    if file_name2 in files:
        with open(f'omni2_{year+1}.dat', 'wb') as f:
            ftps.retrbinary(f'RETR {file_name2}', f.write)
        parse_and_convert_to_csv(file_name2,f'omni_{year+1}_low_res.csv')

    with open(f'omni2_{year}.dat', 'wb') as f:
        ftps.retrbinary(f'RETR {file_name}', f.write)

    ftps.quit()

    parse_and_convert_to_csv(file_name,f'omni_{year}_low_res.csv')
...
def parse_and_convert_to_csv(input_file, output_file):
    """ Parse the data file and convert it to a CSV with only Datetime, Kp, and DST Index columns. """
...
def OMNI_to_csv(year):
    ...
    data = pd.read_csv(f'{year}_OMNI_5m_with_TS05_variables.dat', delim_whitespace=True, header=None)
    ...
    data['datetime'] = [
        datetime(int(year), 1, 1) + pd.to_timedelta(int(day) - 1, unit='d') + \
    ...
    csv_filename = f'{year}_TSY_Data.csv'
...
def Combine(TSYfile, Dstfile, year):
    ...
    futurefile = f'omni_{year+1}_low_res.csv'
    ...
    destination_folder = os.path.join(os.getcwd(), "Parameters", "functions", "ServerData")
