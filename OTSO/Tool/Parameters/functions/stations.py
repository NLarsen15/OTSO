...
class Stations:
 def __init__(self, Stations, Altitude, Zenith, Azimuth):
    ...
    subfolder_name = 'Parameters//functions'
    csv_file_name = 'StationList.csv'

    csv_file_path = os.path.join(current_directory, subfolder_name, csv_file_name)
 ...
 def find_non_matching_stations(self):
    ...
    if len(non_matching_stations) > 0:
      ...
      To Fix:\n\
            ...
            Add the station to the StationList.csv within the Parameters/funtions folder\n\
            Manually add the station within the _param folder\n", sep='\n') 
